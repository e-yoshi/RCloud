                    ...
                    http::Response* pResponse)
{
	...
	{
		std::cout<<"Got here"<< request.uri() <<"\n";
		
		//First time log in, no credentials, index
		if(request.uri().empty()||request.uri().compare("/")==0){
			std::cout<<"Got here2 \n";
			
			// get the path to the homepage file
			std::map<std::string,std::string> variables;
			
			Options& options = server::options();
			FilePath wwwPath(options.wwwLocalPath());
			FilePath homepagePath = wwwPath.complete("homepage.htm");
			
			text::TemplateFilter filter(variables);
			
			pResponse->setFile(homepagePath, request, filter);
            ...
            http::Response* pResponse)
{
   ...
   Options& options = server::options();
   FilePath wwwPath(options.wwwLocalPath());
   FilePath signInPath = wwwPath.complete("encrypted-sign-in.htm");
